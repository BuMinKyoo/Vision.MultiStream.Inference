...
스트림 추가 시 **디코딩**과 **표시기**를 따로 고른다.

| 디코딩 | 설명 |
|---|---|
| **SW** | CPU(FFmpeg) 디코딩 → YUV420P |
| **HW** | GPU(D3D11VA) 디코딩 → NV12 텍스처. **현재 컴포지터 표시기와만 조합 가능** |

| 표시기 | 설명 | SW | HW |
|---|---|---|---|
| **개별(비트맵)** | 스트림마다 `WriteableBitmap` 에 CPU 로 그리기 (D3D 불필요, 가장 느림) | ✅ | ❌ |
| **개별(D3D)** | 스트림마다 `D3DImageYuvPresenter`(D3D9 셰이더) 1개 → 셀별 D3DImage | ✅ | ❌ |
| **컴포지터** | 단일 `StreamCompositor` 가 전 스트림을 한 surface 에 합성 → D3DImage 1개. HW 는 디코드 텍스처를 CPU 다운로드 없이 GPU 안에서 슬롯 텍스처로 복사 | ✅ | ✅ |
...
- **자동 폴백은 없다.** 컴포지터 초기화에 실패하면(GPU/드라이버 부재 등) 컴포지터 모드를 선택할 수 없고, 개별(D3D) 표시기가 실패하면 상태 메시지로 알린다. D3D 를 쓸 수 없는 PC(3D 가속 없는 VM 등)는 **SW + 개별(비트맵)** 을 고른다.
...
YOLO가 **사람(COCO class 0)** 을 검출한 프레임만 로컬 VLM에 넘겨 "사람들이 무엇을 하는지"를 한 문장(한국어)으로 묘사하고, 타일 하단에 자막으로 표시한다. 스트림별 **💬 토글**(또는 좌측 '전체 VLM' 버튼)로 켜고 끈다. 추론(🧠)이 켜져 있을 때만 동작하며, 새 스트림은 기본 OFF.
...
- **디바이스 토글(전역)**: 상단 툴바에서 VLM CPU / GPU 선택. Ollama는 모델을 한 번만 로드하므로 디바이스는 앱 전역 공통이다.
...
상단에 **성능 상태바**(CPU% / RAM MB / GC Gen0~2 / GC Pause)가 항상 떠 있고, 아래에 **RTSP 탭**이 있다.
...
│──────────────────────────│                                      │
│  스트림 목록             │  각 타일(오버레이):                   │
│   ● cam1  추론:CPU SW·개별│   상단바: 이름/추론/디코딩·표시/상태  │
│     🎥 🔊 🧠 💬 ▶ ⏹ ✕   │          + 🎥🔊🧠💬 토글 + ▶⏹✕       │
│──────────────────────────│   검출 박스 (Yellow)                  │
│  전체 영상 🎥 ON/OFF     │   하단: 💬 VLM 자막                   │
│  전체 소리 🔊 ON/OFF     │        + 화면/추론 FPS                │
│  전체 추론 🧠 ON/OFF     │        + 전/추론/후처리 ms            │
│  전체 VLM 💬 ON/OFF      │        + 오디오 fill/drop             │
│  ▶전체 ⏹전체 ✕전체삭제   │                                      │
│  등록: N개               │                                      │
└──────────────────────────┴──────────────────────────────────────┘
```

- **레이아웃**: 스트림 수에 따라 자동 N×N 정사각 격자(N개 → ⌈√N⌉×⌈√N⌉).
- **디코딩 / 표시기**: SW·HW 와 개별(비트맵)·개별(D3D)·컴포지터를 따로 선택. HW 는 컴포지터만 가능하며, HW·컴포지터는 컴포지터 사용 가능 시 활성.
- **추론 디바이스**: 현재 빌드에서 사용 가능한 옵션만 활성화(§8). '추론 사용' 체크로 추론 루프 ON/OFF.
- **VLM**: 상단 툴바에서 CPU/GPU 디바이스(전역) 선택, 스트림별 💬 토글로 ON/OFF (새 스트림은 기본 OFF).
- **토글**: 스트림별 🎥(영상) / 🔊(소리) / 🧠(추론) / 💬(VLM) 개별 ON/OFF, 전체 버튼도 별도 제공.
